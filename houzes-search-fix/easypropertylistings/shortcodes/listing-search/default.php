...
$selected_post_types[] = 'property_sold';
global $epl_settings;
...
if ( ! empty( $selected_post_types ) ) :
	if ( count( $selected_post_types ) > 1 ):
		...
		foreach ($selected_post_types as $post_type ) :
			...
			if ( isset($_GET['action'] ) && $_GET['action'] == 'epl_search' ) {
				if ( $queried_post_type ==  $post_type && isset($_GET['epl_property_status']) && $_GET['epl_property_status'] != 'sold') {
			...
			if($post_type == 'property_sold'){
				$post_type_label = 'Sold';
			}

			if($post_type == 'property_sold' && isset($_GET['epl_property_status'])  && $_GET['epl_property_status'] == 'sold'){
				$post_type_label = 'Sold';
				$is_sb_current = 'epl-sb-current';
			}
	...
	foreach ( $selected_post_types as $post_type ) :
		...
		$property_status_back = $property_status;
		$post_type_tab = '';

		if($post_type == 'property_sold'){
			$post_type = 'property';
			$property_status = 'sold';
			$post_type_tab = 'property_sold';
		} else {
			$property_status = $property_status_back;
		}

		if ( isset($_GET['action'] ) && $_GET['action'] == 'epl_search' ) {
			if($queried_post_type ==  $post_type  && $post_type_tab == 'property_sold') {
				$is_sb_current = 'epl-sb-current';
			} elseif ( $queried_post_type ==  $post_type && $_GET['epl_property_status'] != 'sold') {
				$is_sb_current = 'epl-sb-current';
			} else {
				$is_sb_current = '';
			}
		...
		<form method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			...
			<?php
				...
				foreach ( $epl_frontend_fields as &$epl_frontend_field ) {
					...
					if($epl_frontend_field['key'] == 'property_status'){
						$epl_frontend_field['meta_key'] = 'epl_property_status';
					}
